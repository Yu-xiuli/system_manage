<template>
  <div class="app-container">
    ...
    <el-dialog :title="isEdit?'编辑用户':'添加用户'" :visible.sync="dialogVisible" width="40%">
      <el-form :model="admin" ref="admin" label-width="150px" size="small">
        <el-form-item label="帐号：">
          <el-input v-model="admin.admin" style="width: 250px"></el-input>
        </el-form-item>
        ...
        <el-form-item label="是否启用：">
          <el-radio-group v-model="admin.adminstatus">
...
<script>
import { getUmsadminList } from "@/api/ums";
import {
  createAdmin,
  updateAdmin,
  updateStatus,
...
export default {
  ...
  methods: {
    ...
    handleSearchList() {
      ...
      SearchAdmin({keyword:this.listQuery.keyword}).then((response)=>{
    ...
    handleStatusChange(index, row) {
      this.$confirm("是否要修改该状态?", "提示", {
        ...
        .then(() => {
          updateStatus(row.id, { status: row.adminstatus }).then((response) => {
            this.$message({
              type: "success",
              message: "修改成功!",
            });
    ...
    handleDelete(index, item) {
      this.$confirm("是否要删除该用户?", "提示", {
        ...
        deleteAdmin({ id: item.id }).then((response) => {
          this.$message({
            type: "success",
            message: "删除成功!",
          });
    ...
    getAdList() {
      ...
      getUmsadminList(this.listQuery).then((response) => {
