  ...
  <el-card class="form-container" shadow="never">
    <div v-for="(cate,index) in allResourceCate" :class="index===0?'top-line':null" :key="'cate'+cate.id">
      ...
      <el-row class="table-layout">
        <el-col :span="8" v-for="resource in getResourceByCate(cate.id)" :key="resource.categoryId" style="padding: 4px 0">
...
//   import {fetchAllResourceList} from '@/api/resource';
  ...
  import { getUmsResourceList } from "@/api/ums";
  export default {
    name: "allocResource",
    data() {
      return {
    ...
    methods: {
      getAllResourceList() {
        getUmsResourceList().then(response => {
          console.log(response)
          this.allResource = response.data.items;
      ...
      getAllResourceCateList() {
        listAllCate().then(response => {
            //   console.log(response)
          this.allResourceCate = response.data.items;
      ...
      getResourceByRole(roleId){
        listResourceByRole(roleId).then(response=>{
          let allocResource = response.data.items;
