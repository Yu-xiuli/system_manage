    ...
    <div class="table-container">
      <el-table ref="roleTable"
        ...
        <el-table-column label="用户数"  width="100" align="center">
          <template slot-scope="scope">{{scope.row.adnum}}</template>
        </el-table-column>
...
import { getUmsRoleList } from '@/api/ums'
...
export default {
  ...
  methods:{
    handleResetSearch() {
      ...
      handleDelete(index, item) {
        this.$confirm('是否要删除该角色?', '提示', {
          ...
          deleteRole({ id: item.id }).then(response => {
            this.$message({
              type: 'success',
              message: '删除成功!'
            });
    ...
    getRoList() {
        ...
        getUmsRoleList(this.listQuery).then(response => {
