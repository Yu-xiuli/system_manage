  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="coupon"
      ...
      <el-form-item label="使用门槛：" prop="min_point">
        <el-input v-model.number="coupon.min_point" placeholder="只能输入正整数" class="input-width">
          <template slot="prepend">满</template>
          <template slot="append">元可用</template>
        </el-input>
      </el-form-item>
      <el-form-item label="有效期：">
        <el-date-picker type="date" placeholder="选择日期" v-model="coupon.start_time" style="width: 150px"></el-date-picker>
...
<script>
  import { createCoupon} from "@/api/couponList"
  const defaultCoupon = {
    id:"",
    type: 0,
    name: null,
    ...
    methods:{
      onSubmit(formName) {
       this.$refs[formName].validate(async valid => {
        if (valid) {
          let result = await createCoupon(this.coupon);
